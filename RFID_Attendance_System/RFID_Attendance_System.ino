...
const String sheet_url = "https://script.google.com/macros/s/AKfycbyYbvaygNjRk7PXfXhoBwqzBOKIlRSrPL5TSSkAswuiXLA6ofeHimD-eKXeGTk1ePs6mw/exec?name=";  //Enter Google Script URL
...
//-----------------------------------------
#define WIFI_SSID "iPhone(Madina)"  //Enter WiFi Name
#define WIFI_PASSWORD "11111111"  //Enter WiFi Password
//-----------------------------------------
...
void setup()
{
  ...
  // Initialize I2C explicitly on NodeMCU pins used (SDA=D2, SCL=D1)
  Wire.begin(D2, D1);

  // Use the init() style (matches your working YWRobot example)
  lcd.init();
  lcd.backlight();
  lcd.clear();
  lcd.setCursor(0, 0);
...
{
  //--------------------------------------------------
  // Show prompt once (avoid flicker)
  lcd.clear();
  lcd.setCursor(0, 0);
  ...
  // Build safe printable string from readBlockData and print to serial
  String name = "";
  for (int j=0 ; j<16 ; j++)
  {
  ...
  // code for detecting unknown unregistered  rfid card

  if (name.length() == 0) {
    Serial.println("Unknown card! No name stored.");
  ...
  // Update LCD once (no clearing inside loop)
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("Hey ");
  lcd.print(name);
